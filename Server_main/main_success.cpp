...
static StorageMonitor quota("data/quotas.txt", 500);
static std::mutex coutMux;

// Recursively calculate total size in bytes of all files inside a folder
...
void clientHandler(SOCKET sock) {
    ...
    while (true) {
        int receivedLen = recv(sock, buffer, BUFFER_SIZE - 1, 0);
        if (receivedLen <= 0)
        ...
        //----------------------  DOWLOAD---------------------------------------
            ...
            // Send file header with FILE prefix
...
int main() {
    ...
    safePrint("Server started, waiting for connections...");
